defmodule Synapsis.Agent.ToolDispatcher do
  ...
  def classify(tool_uses, session, monitor, available_tools \\ :all) do
    monitor =
    ...
    classified =
      Enum.map(tool_uses, fn tool_use ->
        permission =
          if tool_available?(tool_use.tool, available_tools) do
            Synapsis.Tool.Permission.check(tool_use.tool, session)
          else
            :denied
          end
  ...
  defp tool_available?(_tool_name, :all), do: true

  defp tool_available?(tool_name, available_tools) when is_list(available_tools),
    do: tool_name in available_tools

  defp tool_available?(_tool_name, _available_tools), do: false
